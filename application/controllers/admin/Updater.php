...
public function index($chk = null)
{
    ...
    // Comment out or remove the update check logic
    /*
    if ($chk == "") {
        ...
        if (!$this->session->flashdata('message') && !$this->session->flashdata('error')) {
            ...
            if ($this->session->has_userdata('version')) {
                ...
                // Handle case if no version data
            }
        }
    }
    */
...
{
    // Disable the update check logic
    /*
    $version  = "";
    ...
    */
    ...
    // Simply return an empty response or a static version
    return json_encode(array('version' => ''));
}
